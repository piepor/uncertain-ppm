...
def plot_case(acc_single, input_data, u_t, target_data, vocabulary_plot,
        out_prob, out_prob_tot_distr, model_type, title_text, target_data_case, case_chosen, save_path):
        ...
        if target_data_case[num_row, 0] == case_chosen:
            ...
            for j in range(target_data.shape[1]):
                ...
                if not target_data[num_row, j].numpy() == 0:
                    ...
                    fig.show(renderer='chromium')
                    file_path = save_path / f"case{case_chosen}-{''.join(title_text.split())}-step{j}.html"
                    fig.write_html(file_path)
                    file_path = save_path / f"case{case_chosen}-{''.join(title_text.split())}-step{j}.svg"
